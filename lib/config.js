...
var nconf = require('nconf'),
    ...
    SmartConfig = require('./smart-config'),
    _ = require('lodash');
...
exports.register = function (plugin, options, next) {
    ...
    if(process.env.NODE_ENV === 'test' || options.testMode) {
        plugin.log(['plugin', 'debug'], "Overriding config in test mode");

        var testOverridesFile = nconf.get('testOverrides');
        if(!testOverridesFile) {
            if(fs.existsSync(path.resolve('./test-config.yaml'))) {
                testOverridesFile = path.resolve('./test-config.yaml');
            }
        }
    ...
    else if(fs.existsSync(path.resolve('./config.yaml'))) {
    ...
    // Provide some dynamic configuration entries
    if(nconf.get('pkg')) {
        try {
    ...
    plugin.expose('CurrentConfiguration', SmartConfig(nconf));
    plugin.expose('loadConfig', exports.loadConfig);
    plugin.expose('wrapAsConfig', exports.wrapAsConfig);
...
exports.loadConfig = function(configPath) {
    ...
    if(fs.existsSync(path.resolve(configPath))) {
        ...
        return SmartConfig(cfg);
    }
};

exports.wrapAsConfig = function(cfg) {
    "use strict";
    var store = new nconf.Provider();
    store.defaults(cfg);
    return SmartConfig(store);
};
